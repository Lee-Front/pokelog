/**
 * Single-attacker move execution (`executeFight`) extracted from pvp-room.ts.
 *
 * This is the giant per-turn routine that handles EVERY step of a single
 * attack: transformations (mega/gmax/dynamax/tera/ultra burst), pre-attack
 * gates (protect, flinch, disable/encore/taunt, sleep/pre-check, two-turn
 * moves), registry dispatch (customResolve / beforeMove / applyEffect /
 * heal), damage calculation (fixed and multi-hit, with item/ability
 * multipliers, screens, terrain, substitute, endure, focus-sash), on-hit
 * and contact effects, ailment/stat application, weather/terrain/hazard
 * setters, form changes, and post-move switching (U-Turn / Baton Pass /
 * phazing).
 *
 * Semantics are preserved byte-for-byte vs the prior inline implementation.
 * pvp-room.ts orchestrates turn order and calls into this for each attacker.
 */
import { calculateDamage, defaultStatStages, applyStatChanges, calculateAccuracy, computeStab } from "../game/battle.js";
...
import { getWeatherTypeModifier, getDefaultWeatherTurns } from "../game/weather.js";
import {
  checkPostAttackForm, checkHpThresholdForm, checkMoveForm,
